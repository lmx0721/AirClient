        ...
        for (char in reFormat.toCharArray()) {
            ...
            if (pos >= 0) {
                ...
                numberList.forEachIndexed { index, num ->
                    if ((fontRend.height.toFloat() + 2F) * index >= expectAnimMin && (fontRend.height.toFloat() + 2F) * index <= expectAnimMax) {
                        fontRend.drawString(num, initX + moveX[indexX], (fontRend.height.toFloat() + 2F) * index, color, shadow)
                    }
                }
                GL11.glTranslatef(0F, -initY + moveY[indexY], 0F)
            } else {
