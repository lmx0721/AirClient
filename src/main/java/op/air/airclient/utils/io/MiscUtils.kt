...
object MiscUtils : MinecraftInstance {
    ...
    fun generateCrashInfo(): String {
        var base = """
            --- Game crash info ---
            Client: ${AirClient.CLIENT_NAME} ${AirClient.clientVersionText} (${AirClient.clientCommit})
    ...
    fun Throwable.showErrorPopup(
        ...
        val openIssueButton = JButton("Open GitHub Issue").apply {
            addActionListener {
                showURL("${AirClient.CLIENT_GITHUB}/issues/new?template=bug_report.yml&title=%5BBUG%5D+Game+crashed+$exceptionType")
            }
        }
