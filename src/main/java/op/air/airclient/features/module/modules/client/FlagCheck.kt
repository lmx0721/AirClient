...
import op.air.airclient.utils.render.RenderUtils.drawPosBox
...
object FlagCheck : Module("FlagCheck", Category.CLIENT, gameDetecting = true) {
    ...
    val onPacket = handler<PacketEvent> { event ->
        ...
        if (packet is S08PacketPlayerPosLook) {
            ...
            if (deltaYaw > 90 || deltaPitch > 90) {
                ...
                flagCount++
                chat("§d检测到 §3强制转头 §e(${deltaYaw.roundToLong()}° | ${deltaPitch.roundToLong()}°) §b(§c${flagCount}x§b)")
            } else {
            ...
            if (!forceRotateDetected) {
                ...
                flagCount++
                chat("§d检测到 §3坐标拉回 §b(§c${flagCount}x§b)")
            }
    ...
    val onUpdate = handler<UpdateEvent> {
                ...
                blockPlacementAttempts.entries.removeIf { (blockPos, timestamp) ->
                    if (currentTime - timestamp > ghostBlockDelay) {
                        ...
                        if (block == Blocks.air && successfulPlacements.contains(blockPos)) {
                            flagCount++
                            chat("§d检测到 §3幽灵方块 §b(§c${flagCount}x§b)")
                            successfulPlacements.clear()
        ...
        if (invalidReason.isNotEmpty()) {
            ...
            chat("§d检测到 §3数据异常 §e$reasonString §b(§c${flagCount}x§b)")
            invalidReason.clear()
        }
        ...
        if (rubberbandReason.isNotEmpty()) {
            ...
            chat("§d检测到 §3运动异常 §8(§e$reasonString§8) §b(§c${flagCount}x§b)")
    ...
    val onRender3D = handler<Render3DEvent> {
        ...
        val text = " 上一个拉回位置: ${remainingTime}s"
