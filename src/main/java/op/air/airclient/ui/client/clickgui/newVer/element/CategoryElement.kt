...
class CategoryElement(val category: Category): MinecraftInstance() {
    ...
    init {
        AirClient.moduleManager.filter { it.category == category }.forEach { moduleElements.add(ModuleElement(it)) }
    }
    ...
    fun drawPanel(mX: Int, mY: Int, x: Float, y: Float, width: Float, height: Float, wheel: Int, accentColor: Color) {
        ...
        Fonts.font52.drawString("${ChatFormatting.GRAY}Modules > ${ChatFormatting.RESET}$name", x + 10F, y + 10F, -1)
        Fonts.font30.drawString("$name", x - 190F, y - 12F, -1)
    ...
    fun handleMouseDrag(mX: Int, mY: Int, mouseButton: Int, x: Float, y: Float, width: Float, height: Float) {
        var mouseX = mX
        var mouseY = mY
        if (mouseY < y + 50F || mouseY >= y + height)
            mouseY = -1
        ...
        if (mouseButton == 0)
            for (moduleElement in moduleElements) {
                moduleElement.handleDrag(mouseX, mouseY, x, startY + animScrollHeight, width, 40F)
                startY += 40F + moduleElement.animHeight
            }
    }
    ...
    fun isAnyKeybindListening(): Boolean {
        for (moduleElement in moduleElements)
