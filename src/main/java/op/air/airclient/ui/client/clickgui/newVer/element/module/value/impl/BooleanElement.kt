/*
 * AirClient++ Hacked Client
 * A free open source mixin-based injection hacked client for Minecraft using Minecraft Forge.
 * https://github.com/PlusPlusMC/LiquidBouncePlusPlus/
 */
package op.air.airclient.ui.client.clickgui.newVer.element.module.value.impl

import op.air.airclient.ui.client.clickgui.newVer.element.components.Checkbox
import op.air.airclient.ui.client.clickgui.newVer.element.module.value.ValueElement
import op.air.airclient.ui.font.Fonts
import op.air.airclient.utils.MouseUtils
import op.air.airclient.config.BoolValue

import java.awt.Color

class BooleanElement(val savedValue: BoolValue): ValueElement<Boolean>(savedValue) {
    private val checkbox = Checkbox()

    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color, accentColor: Color): Float {
        checkbox.state = savedValue.get()
        checkbox.onDraw(x + 10F, y + 5F, 10F, 10F, bgColor, accentColor)
        Fonts.font40.drawString(savedValue.name, x + 25F, y + 10F - Fonts.font40.FONT_HEIGHT / 2F + 2F, -1)
        return valueHeight
    }

    override fun onClick(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float): Boolean {
        if (isDisplayable() && MouseUtils.mouseWithinBounds(mouseX, mouseY, x, y, x + width, y + 20F)) {
            savedValue.set(!savedValue.get())
            return true
        }
        return false
    }
}